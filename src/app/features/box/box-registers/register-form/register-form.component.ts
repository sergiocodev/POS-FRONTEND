...
export class RegisterFormComponent implements OnInit {
    ...
    @Input() set registerId(id: number | null) {
        if (id) {
            this.isEdit.set(true);
            this.currentRegisterId.set(id);
            this.loadRegister(id);
        } else {
            this.isEdit.set(false);
            this.currentRegisterId.set(null);
    ...
    currentRegisterId = signal<number | null>(null);
    ...
    onSubmit(): void {
        ...
        const request$ = this.isEdit()
            ? this.cashService.updateRegister(this.currentRegisterId()!, request)

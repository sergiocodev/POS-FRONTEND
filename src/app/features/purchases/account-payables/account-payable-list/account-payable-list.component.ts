...
export class AccountPayableListComponent implements OnInit {
    @Input() payables: AccountPayableResponse[] = [];
    @Input() isLoading = false;
    @Input() totalElements = 0;
    @Input() pageSize = 10;
    @Input() currentPage = 0;

    @Output() payAction = new EventEmitter<AccountPayableResponse>();
    @Output() viewAction = new EventEmitter<AccountPayableResponse>();
    ...
    columns: TableColumn[] = [
        ...
        { key: 'purchaseIdentifier', label: 'Compra', filterable: true, width: '150px' },
    ...
    onAction(event: { action: string, row: AccountPayableResponse }) {
        if (event.action === 'pay') {
